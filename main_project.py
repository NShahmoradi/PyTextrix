...
def error_management_and_processes(ignored_words_result_txt=[".", ",", "?", "!", ":", ";", "\"", "'", "-", "—", "(", ")", "[", "]", "...", "/", "{", "}", "<", ">", "|", "\\","\n"],
                                   ...
                                   result_of_sort_consecutive_words_counter={}, result_dict_for_normal_pattern={}): 
        ...
        while k < len(questions):
            ...
            if k == 1:
                ...
                if answer in 'dD':
                    ...
                    sorted_dict_decrease = dict(sorted(result_of_sort_consecutive_words_counter.items(), key=itemgetter(1), reverse=True))
                    ...
                    if answer in 'dD':
                        ...
                        sorted_dict_decrease = dict(sorted(result_dict_for_normal_pattern.items(), key=itemgetter(1), reverse=True))
                ...
                max_answer = int(answer)
            
            elif k == 7:
                min_answer = int(answer)
                if min_answer != 0 and max_answer != 0:
                    range_of_length_of_word = list(range(min_answer, max_answer + 1))
                    all_words_list = [word for word in all_words_list if len(word) in range_of_length_of_word and word not in ignored_words_result_txt]
                    counter_words = len(all_words_list)
